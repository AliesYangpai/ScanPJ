package com.scanpj.work.universal.sound;
...
//                successID = soundPool.load(c, R.raw.barcodebeep, 1);
                successID = soundPool.load(c, R.raw.duchao_access, 1);
                try {
                    Thread.sleep(1000);
        ...
        else  if(type== SoundType.FAILURE)
        {
            if(failureID==-1)
            {
                ...
                failureID = soundPool.load(c, R.raw.duchao_deny, 1);
...
//                                failureID =soundPool.load(c,R.raw.serror,1);
            ...
            if(normalID == -1) {
                ...
                normalID = soundPool.load(c, R.raw.barcodebeep, 1);
                try {
                    Thread.sleep(1000);
                } catch (InterruptedException e) {
                    e.printStackTrace();
